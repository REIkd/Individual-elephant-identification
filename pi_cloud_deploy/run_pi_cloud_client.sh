...
#
# Usage:
#   ./run_pi_cloud_client.sh           # interactive window (q quit, r reset)
#   ./run_pi_cloud_client.sh --service # headless, for systemd
...
HEADLESS_ARGS=()
if [[ "${1:-}" == "--service" ]] || [[ "${HEADLESS:-0}" == "1" ]]; then
...
if [[ ${#HEADLESS_ARGS[@]} -eq 0 ]]; then
  echo "Press q to quit, r to reset session"
else
  echo "Running headless (systemd / --service)"
fi
...
CAM_W="${CAMERA_WIDTH:-1920}"
CAM_H="${CAMERA_HEIGHT:-1080}"

CLIP_ARGS=()
if [[ "${PI_CLIP_ENABLE:-1}" != "0" ]]; then
  CLIP_ARGS+=(--clip-enable)
else
  CLIP_ARGS+=(--no-clip-enable)
fi
...
exec python pi_cloud_client.py \
  ...
  --jpeg-quality "$JPEG_Q" \
  --clip-dir "${PI_CLIP_DIR:-data/pi_clips}" \
  --clip-max-width "${PI_CLIP_MAX_WIDTH:-1920}" \
  --clip-fps "${PI_CLIP_FPS:-15}" \
  --clip-retention-days "${PI_CLIP_RETENTION_DAYS:-3}" \
  "${CLIP_ARGS[@]}" \
  "${HEADLESS_ARGS[@]}"
